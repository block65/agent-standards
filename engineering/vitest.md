...
```ts
// BAD: no await, so the test ends before the assertion runs
...
```ts
// BAD: non-null assertion, no runtime check, unhelpful failure
const member = response.members.find((m) => m.userId === targetId)!;
expect(member.role).toBe("admin");

// GOOD: assert, then read
const member = response.members.find((m) => m.userId === targetId);
...
- **Module replacement:** `vi.mock('./module')` replaces an entire module. Use it sparingly, and only for modules that own a real boundary (file system, network, time). Never to swap out internal logic.
...
```ts
// BAD: memo in module scope, so the graph is rebuilt to reset it; global mutated to inject the client
vi.resetModules();
const { loadSettings } = await import("./settings.ts");
Object.assign(env, { STORE: { fetchDoc } });

// GOOD: cache is a value, so its owner picks the lifetime; built once at startup, as before
...
```ts
// BAD: seeded RNG hides input-dependent bugs
faker.seed(42);
const user = faker.person.firstName();

// GOOD: real randomness lets the system meet inputs it will see in production
const user = faker.person.firstName();
```

```ts
// BAD: snapshot with non-deterministic fields and no property matchers, so it fails every run
expect(user).toMatchInlineSnapshot();
//   id: 'abc-123', createdAt: '2026-05-03T...' change every run

// GOOD: property matchers replace the non-deterministic fields
expect(user).toMatchInlineSnapshot({ id: expect.any(String), createdAt: expect.any(Date) }, `...`);
```

```ts
// BAD: catching to keep the test green
try {
  await migrate();
} catch {
  // swallow
...
// GOOD: the migration must succeed; if it doesn't, the test must fail
await migrate();
await expect(db.tableExists("users")).resolves.toBe(true);
```

```ts
// BAD: polling shared state and hoping
await new Promise((r) => setTimeout(r, 100));
expect(received).toHaveLength(1);

// GOOD: await the signal the production code already emits
