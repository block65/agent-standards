...
- **Audit columns:** Most tables should have `create_time` and `update_time`. These carry column-level defaults, since every writer would otherwise have to remember them, which is what defaults exist to prevent. They are bookkeeping, not business meaning, so the no-defaults rule does not apply.
