...
- **A JSDoc block documents a declaration; a line comment explains a line.** JSDoc attaches to an exported declaration and is what an editor shows on hover, so it says what the thing is for. Everything else — inside a function body, above a local, beside a statement — is `//`. A JSDoc block that documents nothing, floating in a body or sitting above a local `const`, is a line comment written wrong.
...
// ❌ depends on something outside this file
...
// The upstream API repeats items across page boundaries.
```
