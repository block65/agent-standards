...
- **Use the right level.** An expected-but-noted condition is `debug` or `trace`, not `error`. Reserve `error` for things an operator should investigate. The codebase under-leverages levels, so pick deliberately.
...
- **Module constants at the top:** True module-level constants (fixed runtime values, typically `UPPER_SNAKE_CASE`, such as `const MAX_RETRIES = 3` or `const API_VERSION = 'v2'`) go directly below the imports, before any functions, classes, or logic. Don't scatter module-scoped config next to the function that happens to use it.
...
- **Every catch must do one of:**
  1. Manually report with `captureException(err)` or equivalent.
...
- **Never silently swallow rejections.** All of these look defensive; all are bugs:
  - `.catch(() => {})`
  - `.catch(() => undefined)`
  - `.catch(noop)`
...
- **`Promise.withResolvers()` when `resolve`/`reject` escape the executor.** Capturing them in an outer `let`, storing them on an object, or handing them to a caller is the deferred pattern, and typing it takes a `let` plus a definite-assignment or non-null assertion, all of them banned. One call replaces the lot.
...
```ts
// BAD: resolve escapes the executor
...
- **`.toString()` over `String()`:** Use `value.toString()` for string conversion, not `String(value)`. Narrow away `null`/`undefined` first, since `.toString()` throws on them, and that loudness is the point; `String()` silently prints `"undefined"`.
