...
- **Never set explicit IDs**: they should be generated by a build-time plugin.
- **Message declaration:** `<FormattedMessage>` in components where possible; `defineMessage` for single strings used as placeholders/properties; `defineMessages` only for fixed enum lookup tables. Shared messages are exported individually, never grouped for export.
...
- **Server state (TanStack Start):** Route `loader` functions with `createServerFn` own initial route data; components read it via `Route.useLoaderData()`. Never fetch route data through `@tanstack/react-query` on first render; that duplicates the loader and re-fetches what the server already sent. Query is the right tool for what loaders do not cover: mutations, polling, and optimistic updates.
