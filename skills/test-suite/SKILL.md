---
name: test-suite
description: "Run the full vitest unit/integration test suite and Playwright e2e tests, then auto-fix failures iteratively using subagents. Use this skill when the user says 'run all tests', 'run the test suite', 'run e2e', 'fix all tests', 'get tests green', or any variation of running + fixing tests across a monorepo. Supports --env dev (default) and --env staging for e2e target. Also trigger when the user asks to 'check everything works' or 'make sure nothing is broken'."
---

# Test Suite Runner

Run vitest then Playwright e2e sequentially across the repo, using subagents for execution. Fix straightforward failures automatically; report design-level issues back to the user without attempting a fix.

This skill is project-agnostic — it provides the orchestration shape (phased run, fix-then-retry loop, failure classification). The actual commands come from the consuming repo's `AGENTS.md`.
...
- **Vitest:** `<command>` (run from repo root)
...
- **Prerequisites (staging):** any env vars or credentials (e.g. `CF_ACCESS_CLIENT_ID` / `CF_ACCESS_CLIENT_SECRET`)
```
...
- `--vitest-only` — skip e2e, just run vitest
- `--e2e-only` — skip vitest, just run e2e

## Argument handling

Parse the arguments string for flags. If `--e2e-only`, skip Phase 1. If `--vitest-only`, skip Phase 2. If neither, run both.

## Phase 1: Vitest (unit + integration)

Skip this phase if `--e2e-only` was passed.

### Execution

Spawn a subagent to run the vitest command from `AGENTS.md`'s `## Testing` section, from the repo root (the cwd this skill was invoked in). The subagent should:
...
- Run the project's formatter on every file edited (`oxfmt`, `prettier`, whichever AGENTS.md specifies).
...
Skip this phase if `--vitest-only` was passed.
...
Same assessment approach as vitest:
...
If `block65-tools:playwright-debug` is available and a failure looks selector- or timing-related, invoke it before attempting a fix — it reads the harness summaries and gives a real diagnosis on the first try.
...
### Final status
- Vitest: X passed, Y failed (Z design issues) — or "skipped" if `--e2e-only`
- E2E (env=<name>): X passed, Y failed (Z design issues) — or "skipped" if `--vitest-only`
